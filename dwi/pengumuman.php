...
class Pengumuman extends MY_Controller
{
    ...
    function add()
    {
        ...
        // $split = explode(" - ", $this->input->post('tgl_tampil', true));
        // $tgl_tampil = date_create($split[0]);
        // $tgl_tampil = date_format($tgl_tampil,"Y-m-d");
        // //$tete = $this->input->post('tgl_tampil');
        // echo $tgl_tampil;
        
        if ($this->form_validation->run('pengumuman') == true) {
            
            $tgl_tampil = $this->input->post('tgl_tampil',true);
            $replace_tgl = str_replace(" ","",$tgl_tampil);
            ...
            $tgl_tampil = date_format($first_date,"Y-m-d");

            $last_date = substr($replace_tgl,11);
            $last_date = date_create($last_date);
            $tgl_tutup = date_format($last_date,"Y-m-d");
    ...
    function edit($segment_3 = '')
    {
        ...
        //$arr_tgltampil = date_create($pengumuman['tgl_tampil']);
        //$arr_tgltampil = date_format($arr_tgltampil,"d-m-Y");

        //$arr_tgltutup = date_create($pengumuman['tgl_tutup']);
        //$arr_tgltutup = date_format($arr_tgltutup,"d-m-Y");

       // $arr_tgl_tampil = str_replace("-","/",$pengumuman['tgl_tampil']);
        
        $arr_tgl_tampil = date_create($pengumuman['tgl_tampil']);
        $arr_tgl_tampil = date_format($arr_tgl_tampil,"d/m/Y");

        //$arr_tgl_tutup = str_replace("-","/",$pengumuman['tgl_tutup']);
        
        $arr_tgl_tutup = date_create($pengumuman['tgl_tutup']);
        $arr_tgl_tutup = date_format($arr_tgl_tutup,"d/m/Y");

        //$arr_tgl = date_create($arr_tgl_tampil." - ".$arr_tgl_tutup);
        $arr_tgl = $arr_tgl_tampil." - ".$arr_tgl_tutup;
        
        $data['arr_tgl'] = $arr_tgl;
