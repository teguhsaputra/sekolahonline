...
class Tugas_model extends CI_Model
{
    ...
    public function retrieve_all(
        ...
        if (!empty($tanggal_awal)) {
            $where['DATE(tgl_buat) >='] = array($tanggal_awal, 'where');
            $where['DATE(tgl_buat) <='] = array($tanggal_akhir, 'where');
            #$this->db->where('DATE(tugas.tgl_buat) <=', $tanggal_akhir);
        }
