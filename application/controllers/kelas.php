...
class Kelas extends MY_Controller
{
    ...
    private function mapel_kelas_hirarki($view = '', $params = array()){
        ...
        $return = '';
        foreach ($parent as $p) {
            $return .= '<div class="p-2 mb-3 bg-primary rounded" id="parent-'.$p['id'].'"><b>'.$p['nama'].'</b></div>';

            $sub_kelas = $this->kelas_model->retrieve_all($p['id'], !empty($params['sub_id']) ? array('id' => $params['sub_id']) : array());
            foreach ($sub_kelas as $s) {
                $return .= '<div class="p-2 bg-default" id="subkelas-'.$s['id'].'" style="margin-left:25px;margin-bottom:5px;">';

                switch ($view) {
                    default:
                        $return .= '<div class="p-3 mb-3 bg-dark rounded">
                            '.$s['nama'].'&nbsp;&nbsp;'.(($s['aktif'] == 0) ? '<span class="label label-warning">Kelas tidak aktif</span>' : '').'
                            '.(($s['aktif'] == 1) ? '<a href="'.site_url('kelas/mapel_kelas/add/'.$p['id'].'/'.$s['id'].'/'.enurl_redirect(current_url())).'" class="btn btn-primary btn-sm float-right"><i class="fas fa-wrench"></i> Atur Matapelajaran</a>' : '').'
                        </div>';
                        if ($s['aktif'] == 1) {
                                ...
                                $return .= '<tr>
                                    <td style="border-top:0px;">
                                        <div class="btn-group float-right">';
                                            if ($v['aktif'] == 0) {
                                                $return .= '<a class="btn btn-success btn-sm" href="'.site_url('kelas/mapel_kelas/aktifkan/'.$p['id'].'/'.$s['id'].'/'.$v['id'].'/'.enurl_redirect(current_url())).'"><i class="fas fa-ok"></i> Aktifkan</a>';
                                            } else {
                                                $return .= '<a class="btn btn-danger btn-sm" href="#modal-'.$v['id'].'" data-toggle="modal"><i class="fas fa-trash"></i> Hapus</a>';
                                            }
                                        $return .= '</div>
                                        <b>
                                        '.$m['nama'].'
                                        '.(($v['aktif'] == 0) ? '<span class="badge badge-danger"><i class="fas fa-info-circle"></i> Matapelajaran Kelas tidak aktif' : '').'
                                        </b>
                                    
                                        <div id="modal-'.$v['id'].'" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-body">
                                                        <h5 id="myModalLabel">Anda yakin ingin menghapus Matapelajaran Kelas ini?</h5>
                                                    </div>
