...
class Ajax extends MY_Controller
{
    ...
    function post_data($page)
    {
        switch ($page) {
            ...
            case 'list_siswa':
                $kelas_id = $this->input->post('kelas_id', TRUE);
                $retrieve_all = $this->absenjurnal_model->retrieve_all_siswa($kelas_id);
                echo '<tr>
                        <td width="80%" class="text-bold">Nama Siswa</td>
                        <td class="text-bold">Kehadiran</td>
                    </tr>';
                foreach ($retrieve_all as $v) {
                    $m = $this->siswa_model->retrieve($v['siswa_id']);
                    if (empty($m)) {
                        continue;
                    }
                    echo    '<tr>';
                    echo    '<td>'.$m['nama'].'</td>';
                    echo    '<input type="text" name="siswa_id[]" value="'.$m['id'].'" hidden/>';
                    echo    '<td>
                                <div class="form-group">
                                    <div class="custom-control custom-radio">
                                        <input class="custom-control-input" type="radio" value="0" id="hadir['.$m['id'].']" name="kehadiran[]['.$m['id'].']" checked>
                                        <label for="hadir['.$m['id'].']" class="custom-control-label">Hadir</label>
                                    </div>
                                    <div class="custom-control custom-radio">
                                        <input class="custom-control-input" type="radio" value="1" id="sakit['.$m['id'].']" name="kehadiran[]['.$m['id'].']">
                                        <label for="sakit['.$m['id'].']" class="custom-control-label">Sakit</label>
                                    </div>
                                    <div class="custom-control custom-radio">
                                        <input class="custom-control-input" type="radio" value="2" id="izin['.$m['id'].']" name="kehadiran[]['.$m['id'].']">
                                        <label for="izin['.$m['id'].']" class="custom-control-label">Izin</label>
                                    </div>
                                    <div class="custom-control custom-radio">
                                        <input class="custom-control-input" type="radio" value="3" id="alpha['.$m['id'].']" name="kehadiran[]['.$m['id'].']">
                                        <label for="alpha['.$m['id'].']" class="custom-control-label">Alpha</label>
                                    </div>
                                </div>
                            </td>';
                    echo    '</tr>';
                        ...
                        echo '<tr>
                                <td colspan="2">
                                    <div class="btn-group float-right">
                                        <input type="submit" value="Simpan Jurnal & Absen" id="lanjut_absen" class="btn btn-success"/>
                                    </div>
                                </td>
                            </tr>';
                    }
            ...
            case 'list_siswa2': // untuk edit absen siswa
                $kelas_id = $this->input->post('kelas_id', TRUE);
                $ma_id = $this->input->post('ma_id', TRUE);
                $tanggal_buat = $this->input->post('tanggal_buat', TRUE);
                $tanggal_buat = date_create($tanggal_buat);
                $tanggal_buat = date_format($tanggal_buat,"Y-m-d");
                $retrieve_all = $this->absenjurnal_model->retrieve_all_absensiswa($ma_id,$tanggal_buat);
                //echo $tanggal_buat; // ERRRRRORRRRRRR NYA TANGGAL NYA KE HARUS DI BALIK DULU GUHHHHH KE DATE_CREATE
                //$retrieve_all = $this->absenjurnal_model->retrieve_all_siswa($kelas_id);
                echo '<tr>
                        <td width="80%" class="text-bold">Nama Siswa</td>
                        <td class="text-bold">Kehadiran</td>
                ...
                foreach ($retrieve_all as $v) {
                    ...
                    echo    '<tr>';
                    echo    '<td>'.$v['nama'].'</td>';
                    echo    '<input type="text" name="siswa_id[]" value="'.$v['siswa_id'].'" hidden/>';   
                   # echo    '<input type="text" name="id_absen" value="'.$v['absen_id'].'" />';   
                    echo    '<td>
                                <div class="form-group">
                                    <div class="custom-control custom-radio">
                                        <input class="custom-control-input" type="radio" value="0" id="hadir['.$v['siswa_id'].']" name="kehadiran[]['.$v['siswa_id'].']" '.$hadir.'> <input type="text" name="id_absen[]" value="'.$v['absen_id'].'" hidden/>
                                        <label for="hadir['.$v['siswa_id'].']" class="custom-control-label">Hadir</label>
                                    </div>
                                    <div class="custom-control custom-radio">
                                        <input class="custom-control-input" type="radio" value="1" id="sakit['.$v['siswa_id'].']" name="kehadiran[]['.$v['siswa_id'].']" '.$sakit.'>
                                        <label for="sakit['.$v['siswa_id'].']" class="custom-control-label">Sakit</label>
                                    </div>
                                    <div class="custom-control custom-radio">
                                        <input class="custom-control-input" type="radio" value="2" id="izin['.$v['siswa_id'].']" name="kehadiran[]['.$v['siswa_id'].']" '.$izin.'>
                                        <label for="izin['.$v['siswa_id'].']" class="custom-control-label">Izin</label>
                                    </div>
                                    <div class="custom-control custom-radio">
                                        <input class="custom-control-input" type="radio" value="3" id="alpha['.$v['siswa_id'].']" name="kehadiran[]['.$v['siswa_id'].']" '.$alpha.'>
                                        <label for="alpha['.$v['siswa_id'].']" class="custom-control-label">Alpha</label>
                                    </div>
                                </div>
                            </td>';
                    echo    '</tr>';
