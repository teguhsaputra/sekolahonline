...
class Tugas extends MY_Controller
{
    ...
    function index($segment_3 = '')
    {
        ...
        if ($this->form_validation->run('tugas/filter') == true) {
          ...
          // $data['x2'] = $last_date;
            ...
            $filter = array(
                ...
                'last_date'   => $last_date,
                'tanggal_between'  => $tanggal_between,
            );
        ...
        */
        ...
        /*
        $tugas_id           = $results[]          
        = $this->tugas_model->retrieve_nilai(null, $tugas['id'], get_sess_data('user', 'id'));
        */
    ...
    function add($segment_3 = '')
    {
        ...
        $data['mapel']   = $this->mapel_model->retrieve_all_mapel();
        $data['kelas']   = $this->kelas_model->retrieve_all_child();
        # $data['comp_js'] = get_texteditor(); ada di elearning_helper.php di folder helpers
        # panggil datepicker
        $data['comp_js'] = load_comp_js(array(
            base_url('assets/comp/moment/moment.min.js'),
        ...
        if ($this->form_validation->run($form_validation) == TRUE) {
            ...
            $tanggal_ujian = date_create($tanggal_ujian);
            $tanggal_ujian = date_format($tanggal_ujian,"Y-m-d H:i:s");
            $judul    = $this->input->post('judul', TRUE);
    ...
    function edit($segment_3 = '', $segment_4 = '')
    {
        ...
        $tanggal_ujian = $tugas['tgl_buat'];
        $tanggal_ujian = date_create($tanggal_ujian);
        $data['tanggal_ujian'] = date_format($tanggal_ujian,"d-m-Y H:i:s");

        $data['type']        = $tugas['type_id'];
        $data['tugas']       = $tugas;
    ...
    function kerjakan($tugas_id = '')
    {
        ...
        #ambil nama kelas
        $gki = $this->kelas_model->retrieve_siswa(null, array(
                'siswa_id' => get_sess_data('user', 'id'),
                'aktif'    => 1
            ));
        $gnk = $gki['kelas_id']; 
        $kelas = $this->kelas_model->retrieve($gnk);
        $data['kelas_aktif'] = $kelas['nama'];
    ...
    function submit_upload($tugas_id = '', $unix_id = '')
    {
        ...
        if (!empty($check_field)) {
            ...
            $this->session->set_flashdata('tugas', get_alert('success', 'File Berhasil di Upload, Klik Link Ujian Untuk Melaksanakan Ujian.'));
    ...
    function nilai($tugas_id = '', $mode = '')
    {
        ...
        if (is_siswa()) {
            ...
            if(empty($nilai)){
                $data['nilai'] = "Belum Dikoreksi";
            }
            else{
                $data['nilai'] = round($nilai['nilai'],2);
            }
