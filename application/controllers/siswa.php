...
class Siswa extends MY_Controller
{
    ...
    function add($segment_3 = '')
    {
        ...
        if ($this->form_validation->run('siswa/add') == TRUE AND !$error_upload) {
            ...
            $this->session->set_flashdata('siswa', get_alert('success', 'Data siswa berhasil disimpan.'.$password));
            redirect('siswa/index/1');
    ...
    function import_siswa()
    {
        ...
        //$config['max_size']      = '0';
        $config['file_name']     = url_title($this->input->post('kelas_id', TRUE).'_'.time(), '_', TRUE);
        $this->upload->initialize($config);
        
        $this->form_validation->set_rules('kelas_id', 'Kelas', 'required|trim|xss_clean');
        //$this->form_validation->set_rules('default_username', 'Default Username', 'required|trim|xss_clean');
        ...
        if ($this->form_validation->run('siswa/import_siswa') == TRUE AND !$error_upload) {
            $kelas_id           = $this->input->post('kelas_id', TRUE);
           // $default_username   = $this->input->post('default_username', TRUE);
            $upload_data        = $this->upload->data();
            $file               =  FCPATH.'userfiles/files/'.$upload_data['file_name'];

            $data_excel = data_excel($file);
            foreach ($data_excel as $Key => $Row)
	        {
                if ($Key < 1) continue;
                
                $tanggal_lahir = date_create($Row[4]);
                ...
                $password = date_create($Row[4]);
                $password = date_format($password,"dmY");
                
                $username = $Row[0]."@aa.sch.id";
                # simpan data login
                $this->login_model->create(
                    $username,
                    $password,
           ...
           $this->session->set_flashdata('siswa', get_alert('success', 'Data siswa berhasil di import.'));
           redirect('siswa');
        ...
        $this->twig->display('import_siswa.html', $data);
    }
}
