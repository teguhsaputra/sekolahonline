...
class Login extends MY_Controller
{
    function index()
    {
        ...
        $data['comp_css'] = load_comp_css(array(
            base_url('assets/comp/paralaxbackground/normalize.css'),
        ));
        ...
        $data['site_key'] = $this->config->item('google_key');
        */
        ...
        if ($this->form_validation->run('login') == TRUE) {
            ...
            if (!$status['success']) {
                ...
                //exit;
                $this->session->set_flashdata('login', get_alert('warning', 'Maaf Google Recaptcha tidak sukses.'));
                redirect('login');
            }
            // GOOGLE RECAPTCHA
            */
            ...
            if (empty($get_login)) {
                ...
                if ($user['status_id'] != 1) {
                    $this->session->set_flashdata('login', get_alert('warning', 'Maaf Anda belum bisa mengikuti ujian, Silahkan hubungi bagian administrasi (keuangan).'));
                    redirect('login');
                }
    ...
    function login_log($segment_3 = "", $segment_4 = "")
    {
        ...
        # ambil data nama
        $retrieve_nama = $this->login_model->retrieve($login_id);
        ...
        $data['nama'] = $retrieve_nama['username'];
